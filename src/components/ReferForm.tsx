...
export default function ReferForm() {
    ...
    const [referCountry, setReferCountry] = useState<string>("ae");
    ...
    const validationSchema = Yup.object({
        ...
        referrer_phone: Yup.string().required("Referral Phone Number is required")
            .test(
                ...
                "Invalid phone number for selected country",
                function (value) {
                    if (!value || !referCountryCode) return false;
                    const fullNumber = `+${referCountryCode} ${value}`;
                    return isValidPhoneNumber(fullNumber);
                }
            ),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm, setSubmitting }) => {
            try {
                ...
                const referalPhone = (phone: string, countryCode: string) => {
                    const digitsOnly = phone.replace(/\D/g, "");
                    const codeOnly = countryCode.replace(/\D/g, "");
                    const phoneWithoutCode = digitsOnly.startsWith(codeOnly)
                        ? digitsOnly.slice(codeOnly.length)
                ...
                const formDataWithUTM = {
                    ...
                    phone: normalizePhone(referrer_phone, referCountryCode),
        ...
        >
            ...
            <div className="flex justify-center md:justify-start mt-4 gap-4">
                ...
                >Submit</Button>
                {
                    formik.isSubmitting &&
                    (
                        <div id="loader" className="w-7.5 my-auto">
                            <Spinner />
                        </div>
                    )
                }                
