...
export default function BlogList({ posts }: BlogItemsProps){
    return(        
        <Link key={posts?._id} href={`blog/${posts?.slug}`}>
            <div className="blog-card  h-full bg-[#0F0F0F] border border-[#1F1F1F] rounded-10 overflow-hidden shadow-lg">
                <div className="relative w-full">
                    <div className="w-full h-62.5 relative group overflow-hidden">
                        <Image
                            fill
                            alt={posts?.mainImage?.alt || ""}
